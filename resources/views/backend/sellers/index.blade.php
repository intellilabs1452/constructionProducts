            ...
            <table class="table aiz-table mb-0">
                ...
                <tr>
                    ...
                    <th data-breakpoints="lg">{{translate('Approval')}}</th>
                     ...
                     <th data-breakpoints="lg">{{translate('Payment Approval')}}</th>
                ...
                @foreach($sellers as $key => $seller)
                    @if($seller->user != null && $seller->user->shop != null)
                        <tr>
                            ...
                            <td>
                                <label class="aiz-switch aiz-switch-success mb-0">
                                    <input onchange="update_payment_status(this)" value="{{ $seller->user_id }}" type="checkbox" <?php if($seller->user->payment_done == 1 && $seller->user->offline_payment == 1) echo "checked";?> >
                                    <span class="slider round"></span>
                                </label>
                            </td>
                            ...
                            <td><select class="form-control" name="package" id="package" onchange="update_package(this.value,{{$seller->user->id}})">
                                <option value="0">Select Package</option>
                      @foreach (\App\SellerPackage::all() as $brand)
                                <option value="{{ $brand->id }}" @if($seller->user->seller_package_id == $brand->id) selected
...
@section('script')
    <script type="text/javascript">
        ...
        function update_payment_status(el){
            ...
            $.post('{{ route('sellers.paymentstatus') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Payment status updated successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
        ...
        function update_package(value,seller_id)
        {
            
             $.post('{{ route('sellers.updatepackage') }}', {_token:'{{ csrf_token() }}', id:value, seller_id:seller_id}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Approved sellers updated successfully') }}');
                }
                else{
