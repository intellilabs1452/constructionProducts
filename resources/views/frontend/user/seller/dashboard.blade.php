    ...
    <div class="row">
        ...
        <div class="col-md-4">
          <div class="card p-5 text-center">
              <div class="mb-3" style="max-height: 27px;">
                  @if(Auth::user()->seller->verification_status == 0 || Auth::user()->payment_done==0)
                      <img loading="lazy"  src="{{ static_asset('assets/img/non_verified.png') }}" alt="" width="130" style="margin-top: -41px;">
                  @else
                      <img loading="lazy"  src="{{ static_asset('assets/img/verified.png') }}" alt="" width="130" style="margin-top: -41px;">
                  @endif
              </div>
              @if(Auth::user()->seller->verification_status == 0)
                  <a href="{{ route('shop.verify') }}" class="btn btn-primary" hidden>{{ translate('Verify Now')}}</a>
              @endif
          </div>
           <div class="card p-4 text-center">
              <div class="h5 fw-600">{{ translate('Business')}}</div>
              <p>{{ translate('Manage & organize your shop')}}</p>
                 @if(Auth::user()->user_type == 'seller' &&  in_array('6', json_decode($permissions)))
              <a href="{{ route('shops.index') }}" class="btn btn-soft-primary">{{ translate('Go to setting')}}</a>
                 @else
                  <a href="" class="btn btn-soft-primary" style="pointer-events: none;cursor: default;">{{ translate('Go to setting')}}</a>
      ...
      <div class="col-md-4">
          ...
          <div class="card p-4 text-center">
             ...
             <div class="card">
                  ...
                  @php
                      $seller_package = \App\SellerPackage::find(Auth::user()->seller_package_id);
                  @endphp
                  <div class="card-body text-center">
                      ...
                      @endif
                      <div class="text-center">
                          <a href="{{ route('seller_packages_list_show') }}" class="btn btn-soft-primary">{{ translate('Upgrade Package')}}</a>
                      </div>
                  </div>
              </div>
