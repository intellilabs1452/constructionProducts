...
@section('panel_content')
    ...
    <div class="aiz-titlebar mt-2 mb-4">
        <input type="hidden" value="{{ $locations->locations }}" id="nooflocations">
      <div class="row align-items-center">
        <div class="col-md-6">
            <h1 class="h3">{{ translate('Business Settings')}}
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form class="" action="{{ route('shops.update', $shop->id) }}" method="POST" enctype="multipart/form-data">
                ...
                @csrf
                <div class="row">
                    <label class="col-md-2 col-form-label">{{ translate('Business Name') }}<span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <input type="text" class="form-control mb-3" placeholder="{{ translate('Business Name')}}" name="name" value="{{ $shop->name }}" required>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">{{ translate('Business Logo') }}</label>
                    <div class="col-md-10">
                 ...
                 <div class="row">
                    <label class="col-md-2 col-form-label">
                        {{ translate('Location') }}<span class="text-danger text-danger">*</span>
                    </label>
                      <div class="col-md-10" style="margin-bottom:17px">
                            <select class="form-control aiz-selectpicker" data-live-search="true" data-selected-text-format="count" name="location[]" id="location" multiple required data-max-options="{{ $locations->locations}}">
                                    @foreach (\App\Models\SellerLocation::orderBy('name', 'asc')->get() as $key => $location)
                                    <option value="{{ $location->id }}"
                                        data-content="<span><span>{{ $location->name }}</span></span>"  @if (in_array($location->id, json_decode($shop->locations))) selected @endif></option>
                                    @endforeach
                                </select>
                 </div> 
                 </div>
                <div class="row">
                    <label class="col-md-2 col-form-label">
                        {{ translate('Business Phone') }}<span class="text-danger text-danger">*</span>
                    </label>
                    <div class="col-md-10">
                ...
                <div class="row">
                    <label class="col-md-2 col-form-label">{{ translate('Business Address') }} <span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
