    ...
    public function checkout(Request $request)
    {
        if ($request->payment_option != null) {
            ...
            if ($request->session()->get('combined_order_id') != null) {
                ...
                elseif ($request->payment_option == 'onlinepayment') {
                    flash(translate("Your order has been placed successfully"))->success();
                    return redirect()->route('order_confirmed');
                }
...
//        if (Session::has('cart') && count(Session::get('cart')) > 0) {
    ...
    public function store_delivery_info1(Request $request)
    {
        ...
        if ($carts && count($carts) > 0) {
            foreach ($carts as $key => $cartItem) {
                ...
                else if ($request['shipping_type_' . $product->user_id] == 'pickupstore') {
                   $cartItem['shipping_type'] = 'pickup_store';
                } 
                else {
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        if ($carts && count($carts) > 0) {
            ...
            $total = 0;
            $categories = Category::all();
            return view('frontend.shipping_info', compact('categories', 'carts','no_address'));
    ...
    public function order_confirmed()
    {
        ...
        $seller  = Shop::where('user_id', $combined_order->user_id)->first();
        ...
        return view('frontend.order_confirmed', compact('combined_order','seller'));
    }
}
