...
class OrderController extends Controller
{
    ...
    {
       $collection = collect(json_decode(json_encode($request->shipping_type, true)));
       $isInArray = in_array('home_delivery', array_column($collection->toArray(), 'shipping_type'));
...
//echo $product->user_id;die;
            ...
            // save order details
             $shippingTypeOfSeller = $collection->where('owner_id', $product->user_id)->first()->shipping_type;
            ...
            OrderDetail::create([
                ...
                'payment_status' => $set_paid ? 'paid' : 'unpaid',
                'shipping_type'=>$shippingTypeOfSeller
            ]);
            $product->update([
